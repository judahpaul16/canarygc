<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    if (isRegistrationMode) {
      ...
      try {
        await authData.registerAdmin(email, password, confirmPassword);
        goto("/dashboard");
      } catch (err: any) {
        error = err.message;
      }
      ...
      try {
        await authData.loginAdmin(email, password);
        goto("/dashboard");
      } catch (err: any) {
        error = err.message;
      }
...
>
  <div
    class="dashboard w-fit h-fit flex flex-col justify-center p-10 rounded-3xl max-w-[400px]"
  >
        ...
        {#if isRegistrationMode}
          <div class="mt-2">
            <div class="flex justify-start items-center">
              <div class="text-sm min-w-fit mr-3">
              ...
              <div class="w-[70%] h-2 bg-gray-200 rounded">
                <div
                  ...
                  style="width: {(passwordStrength / 4) *
                    100}%; background-color: {getPasswordStrengthColor(
                    passwordStrength
                  )}"
                ></div>
              </div>
            ...
            <div class="text-xs mt-1">
              Password must contain at least 10 characters, including uppercase,
