...
* **Flight controller firmware.** A Firmware tab detects a connected Betaflight or INAV board over the MultiWii Serial Protocol (MSP) and flashes it over USB DFU. It browses the live INAV release and Betaflight target catalogs, downloads and flashes an INAV target hex in place, flashes any Intel HEX (a cloud-built Betaflight or a custom build), and can reboot the board into its DFU bootloader first.
