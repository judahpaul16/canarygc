...
* **Smart path optimization.** One click routes the mission clear of hazards without changing the waypoint order: it inserts detour waypoints that go around restricted airspace and mapped obstacles (a visibility-graph path over the buffered hazard corners) and moves any waypoint that sits inside restricted airspace back outside it, then reports the waypoints added and moved.
