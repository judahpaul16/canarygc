...
* **Mission planner** with a 2D map (Leaflet) and a 3D map (MapLibre). One persistent map renders behind every page; the planner and the dashboard mini-map are live windows into it, and fullscreen expands the same map.
...
* **Live air traffic.** A toggleable ADS-B layer draws nearby aircraft with heading, altitude, and speed, merging traffic reported by the vehicle's onboard receiver (`ADSB_VEHICLE`) with keyless community network feeds (adsb.lol, adsb.fi).
